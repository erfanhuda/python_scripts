...
class ConfigurationTable(ttk.Treeview):
    ...
    def on_double_click(self, e):
        ...
        if column == "#0":
            ...
            edit_box.place(
                x=column_box[0], y=column_box[1], width=column_box[2], height=column_box[3])
...
class ContactsFrame(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.table = ttk.Treeview(
            self, columns=col_names, selectmode='extended')
        ...
        self.table.insert(company_row, values=(
            "Box Kayu", "085799663331", "box_kayu@gmail.com", "Jakarta, Indonesia"), index=tk.END)
        ...
        self.table.bind("<MouseWheel>", lambda e: self.table.yview_scroll(
            int(-1*(e.delta)), "units"))
        self.table.bind("<Control-MouseWheel>",
                        lambda e: self.table.xview_scroll(int(-1*(e.delta)), "units"))
...
class ECL_Menu(ttk.Frame):
    ...
    def create_widgets(self):
        self.tabs = ttk.Notebook(
            self, name="tabnav", width=self.winfo_x(), height=self.winfo_y())
        ...
        self.table_ecl = ttk.Treeview(
            self.frame_one, columns=columns, show='headings')
        ...
        self.table_ecl_scrollbar = ttk.Scrollbar(
            self.table_ecl, orient=tk.HORIZONTAL, command=self.table_ecl.yview)
...
class FL_Menu(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.label_three = ttk.Label(
            self.frame_three, text="This is Frame MFA")
        ...
        self.__open_json_file = ttk.Button(
            self.input_frame, text="Open JSON File", command=self.__open_json_file)
        ...
        self.__save_json_file = ttk.Button(
            self._note_first, text="Save", command=self.__save_json_file)
        self._run_file = ttk.Button(
            self._note_first, text="Run", command=self._run_script)
        self.__save_json_file.pack(
            padx=5, pady=5, side='left', fill='x', expand=False)
        self._run_file.pack(padx=5, pady=5, side='right',
                            fill='x', expand=False)
        ...
        self.__add_button = ttk.Button(
            self.input_frame_one, text="Add", command=self.__add_var_button)
        ...
        col_names = ("",)
        self.var_tables = VariablesTable(
            self.frame_one, columns=col_names, selectmode='extended')
        ...
        yscroll = tk.Scrollbar(
            self.var_tables, orient=tk.VERTICAL, command=self.var_tables.yview)
        xscroll = tk.Scrollbar(
            self.var_tables, orient=tk.HORIZONTAL, command=self.var_tables.xview)
        xscroll.pack(padx=5, pady=5, side='bottom', fill='both')
        yscroll.pack(padx=5, pady=5, side='right', fill='both')
        self.var_tables.configure(
            xscrollcommand=xscroll.set, yscrollcommand=yscroll.set)
        self.var_tables.bind(
            "<Control-MouseWheel>", lambda e: self.var_tables.xview_scroll(int(-1*(e.delta)), "units"))
    ...
    def __open_json_file(self):
        self._file = filedialog.askopenfilename(defaultextension=".json", filetypes=[
    ...
    def __save_json_file(self):
        ...
        if self._file == None:
            ...
            self._file = filedialog.asksaveasfile(
                mode="w", confirmoverwrite=True, defaultextension=".json")
    ...
    def __add_var_button(self):
        ...
        if self._file == "":
            ...
            file = open(self._file, "r", newline="",
    ...
    def _table_config_manual(self):
        ...
        self._export_json = ttk.Button(
            self.btn_frame, text="Export to JSON", command=self._export_to_json)
        self._save = ttk.Button(
            self.btn_frame, text="Save", command=self._on_save_to_json)
        ...
        self.table = ConfigurationTable(
            self._note_second, columns=col_names, selectmode='extended')
        self.table.heading("#0", text="Configurations")
        self.table.heading("values", text="Values")

        variable_config = self.table.insert(
            parent="", iid='1', index=tk.END, text="Set Variables")
        y_var_config = self.table.insert(
            parent=variable_config, iid='11', index=tk.END, text="Y Variable", values=("ODR",))
        self.table.insert(parent=y_var_config, index=tk.END,
                          iid='111', text="z-score", values=("",))
        self.table.insert(parent=y_var_config, index=tk.END,
                          iid='112', text="Ln", values=("",))
        x_var_config = self.table.insert(
            parent=variable_config, iid='2', index=tk.END, text="X Variable", values=("GDP",))
        self.table.insert(parent=x_var_config, index=tk.END,
                          iid='21', text="Moving Average (Month)", values=("",))
        self.table.insert(parent=x_var_config, index=tk.END,
                          iid='22', text="Exponential (Month)", values=("",))
        self.table.insert(parent=x_var_config, index=tk.END,
                          iid='23', text="Lag (Month)", values=("",))
        self.table.insert(parent=x_var_config, index=tk.END,
                          iid='24', text="Lead (Month)", values=("",))
        self.table.insert(parent=x_var_config, index=tk.END,
                          iid='25', text="Delta (Month)", values=("",))
        self.table.insert(parent=x_var_config, index=tk.END,
                          iid='26', text="Variance (Month)", values=("",))
        self.table.insert(parent=x_var_config, index=tk.END,
                          iid='27', text="Growth (Month)", values=("",))

        test_sample_config = self.table.insert(
            parent="", iid='3', index=tk.END, text="Set Samples")
        self.table.insert(parent=test_sample_config, iid='31',
                          index=tk.END, text="Training Date", values=("",))
        self.table.insert(parent=test_sample_config, iid='32',
                          index=tk.END, text="Testing Date", values=("",))

        forecast_config = self.table.insert(
            parent="", iid='4', index=tk.END, text="Forecast")
        self.table.insert(parent=forecast_config, iid='41',
                          index=tk.END, text="Training Date", values=("",))
        self.table.insert(parent=forecast_config, iid='42',
                          index=tk.END, text="Testing Date", values=("",))
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.bar_label = ttk.Label(
            self.bottom_bar, text=str(build_connection()))
    ...
    def create_menu(self):
        ...
        modeling_menu.add_command(
            label="Forward Looking", command=partial(self.open_menu, FL_Menu))
        modeling_menu.add_command(
            label="OJK NPL Buku", command=partial(self.open_menu, OJK_Menu))

        ecl_menu = tk.Menu(self.menu)
        ecl_menu.add_command(
            label="ECL", command=partial(self.open_menu, ECL_Menu))

        setting_menu = tk.Menu(self.menu)
        setting_menu.add_command(
            label="Global Settings", command=partial(self.open_pop_up, SettingMenu))
