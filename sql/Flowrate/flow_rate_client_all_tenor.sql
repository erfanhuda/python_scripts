...
, disburse_daily as (
     select * from (select pt_date, product_type, tenor,payment_freq,tenor_in_month,sum(principal) as principal,sum(principal_mtd) as principal_mtd
     ...
     group by pt_date, product_type, tenor_in_month,tenor,payment_freq) A
