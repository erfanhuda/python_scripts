...
, disburse_daily as (
     select * from (select pt_date, product_type, tenor_in_month, sum(principal) as principal,sum(principal_mtd) as principal_mtd
     ...
     group by pt_date, product_type, tenor_in_month) A
     where A.pt_date = last_day(A.pt_date)
)
, current_to_current as (
     select 
     a.product_type
     ,cast(a.tenor_in_month as integer) as tenor
     ...
     ON add_months(A.pt_date,-1) = B.pt_date AND 
       ...
       A.tenor_in_month = B.tenor_in_month
...
, m1m4_raw_data as (
     ...
     from (select 
          ...
          from (
               ...
               AND A.tenor_in_month = B.tenor_in_month) a
          left join mk_com C
...
, m1_m4 as (
    select 
     product_type
     ,cast(tenor_in_month as integer) as tenor
     , 
...
insert overwrite table id_finpro.flow_rate_all_client_summary_tenor_in_month
