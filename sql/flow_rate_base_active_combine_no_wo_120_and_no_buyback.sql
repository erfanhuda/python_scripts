...
select loan_no,client_no,product_code
        ...
        from compiled_table_buyback
        where pt_date between '{{start_date}}' and '{{end_date}}'
)
, date_series_wo_120 AS (
    select posexplode(split(repeat("o", datediff(current_date(),min(pt_date))), "o")) from id_finpro.master_base_wo_120
) 
, join_date_wo_120 as (
  select distinct pt_date, loan_no, day_past_due from id_finpro.master_base_wo_120 
)
, join_final_wo_120 as (  
  select b.loan_no, date_add(b.pt_date, a.pos) as pt_date, b.day_past_due + a.pos as day_past_due  
  from date_series_wo_120 a
  cross join join_date_wo_120 b
)
, compiled_table_wo_120 as (
  ...
  ,'WRITE_OFF' as import_source
  ,a.pt_date
  from join_final_wo_120 a
  left join id_finpro.master_base_wo_120 b
  on a.loan_no = b.loan_no
  where a.day_past_due <= 181
)
, mk_wo_120 as (
    select loan_no,client_no,product_code
        ,product_type
...
insert overwrite table id_finpro.flow_rate_base_active_combine_wo_120_buyback partition (pt_date)
select * from mk4;